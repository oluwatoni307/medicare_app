...
class MedicationViewModel extends ChangeNotifier {
  ...
  void addScheduleTime(TimeOfDay time) {
    ...
    if (!_medication.scheduleTimes!.contains(time)) {
      ...
      // Sort times
      _medication.scheduleTimes!.sort((a, b) => 
  ...
  List<String> validateMedication() {
    ...
    if (_medication.medicationName == null || _medication.medicationName!.isEmpty) {
    ...
    if (_medication.scheduleTimes == null || _medication.scheduleTimes!.isEmpty) {
  ...
  Future<bool> saveMedication() async {
    try {
      ...
      if (isEditMode && _medicationId != null) {
        ...
        // Add new medication
        _medication.id = null; // Let system auto-generate
        await _medicationService.addMedication(_medication, user.id);
      }
