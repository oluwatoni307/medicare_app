...
class AnalysisViewModel extends ChangeNotifier {
  ...
  DateTime _currentWeekStart = _calculateWeekStart(
    DateTime.now(),
  ...
  Future<void> loadMonthlyData([DateTime? month]) async {
    ...
    try {
      ...
      final data = await AnalysisService().getMonthlyData(
        monthStr,
      ); // No userId needed
      // --- END CHANGE ---
  ...
  Future<void> loadWeeklyData([DateTime? weekStart]) async {
    ...
    try {
      ...
      final data = await AnalysisService().getWeeklyData(
        startDateStr,
        endDateStr,
      ); // No userId needed
  ...
  Future<Map<String, double>> getDailyPieChartData(DateTime date) async {
    ...
    final data = await AnalysisService().getDailyData(
      dateStr,
    ); // No userId needed
    // --- END CHANGE ---
...
// Add to AnalysisViewModel - FIXED weeklyChartData getter

List<ChartDataPoint> get weeklyChartData {
  // FIXED: Use the correct service keys that match what AnalysisService returns
  const serviceDayKeys = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun']; // lowercase, 'thu' not 'thur'
  
  return serviceDayKeys.asMap().entries.map((entry) {
    final index = entry.key;
    final serviceKey = entry.value;
    final date = _currentWeekStart.add(Duration(days: index));
    final value = _weeklyAdherenceData[serviceKey] ?? 0.0; // Use correct service key
  ...
  Future<List<DailyTile>> getDailyTileData(DateTime date) async {
    ...
    final data = await AnalysisService().getDailyData(
      dateStr,
    ); // No userId needed, returns List<DailyTile>
    // --- END CHANGE ---
